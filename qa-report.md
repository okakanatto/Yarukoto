...
- **FAB 追加後の一覧反映**: `layout.js` の FAB から `yarukoto:taskAdded` イベントが dispatch されるが、`TaskList.js` はこのイベントを直接リスンしていない。FAB 経由の場合、ページ遷移やリロードまでは一覧に反映されない可能性がある
- **アーカイブタブでの DnD**: `!showArchived` ガードにより問題なし
...
## STEP R：リグレッションテスト（v1.3.1 リリース前検証 2026-03-01）

### 1. 影響範囲の特定

**【変更サマリーからの特定】**

- **変更内容**: リリース前検証で発見された NG 3件の修正
  - (NG-1) `app/layout.js:79` — `fetchTodayProgress` SQL に `AND archived_at IS NULL` 追加
  - (NG-2) `hooks/useTodayTasks.js:184` — `loadTasks` SQL に `AND t.status_code != 5` 追加
  - (NG-B-1) `app/routines/_components/RoutineFormModal.js:52-56` — `useEffect` + Escape キーリスナー追加

- **影響が想定される箇所（コード参照元の洗い出し結果）**:
  - `app/layout.js` → `fetchTodayProgress()` → サイドバー進捗リング表示（`todayProgress` state, L170-187）
  - `hooks/useTodayTasks.js` → `loadTasks()` → `app/today/page.js`（タスク一覧表示、進捗統計、DnD、フィルタ、ソート全般）
  - `app/routines/_components/RoutineFormModal.js` → `app/routines/page.js:163-170`（モーダル開閉制御）
  - `app/dashboard/page.js` — 独立した SQL クエリを持つが、同様の `status_code != 5 AND archived_at IS NULL` パターンとの一貫性を確認
  - `components/TaskList.js` — 独立した SQL クエリ。`status_code != 5` のグローバル除外は**意図的に持たない**（全ステータス表示がタスク一覧の仕様）
  - `hooks/useTaskActions.js` — `useTodayTasks.js` から `handleRoutineStatusChange` 等を利用するが、SQL 変更の影響は受けない（アクション実行後に `loadTasks` を再呼び出しするのみ）
  - `components/TaskEditModal.js:55-58` — 既存の Escape キーリスナー。`RoutineFormModal` とは別画面で排他的に動作するため干渉なし

### 2. リグレッションテスト結果

#### 第1段階：変更箇所の直接テスト

✅ OK: 3件 全件パス（NG-1修正 / NG-2修正 / NG-B-1修正の各SQL・リスナーが正しく実装されていることを確認）

#### 第2段階：影響範囲のリグレッションテスト

✅ OK: 9件 全件パス（サイドバー・ダッシュボードとの一貫性 / 今日やるタスク画面の統計・表示・バナー / タスク一覧のキャンセル表示維持 / Escape リスナーの干渉なし・競合なし）

### 3. 総合判定

**全項目 OK（NG: 0件）**

今回の v1.3.1 リリース前検証で発見された NG 3件の修正について、変更箇所の直接テスト（3件）および影響範囲テスト（9件）の全12項目でリグレッションなしを確認した。

1. **NG-1 修正**: `layout.js` の `fetchTodayProgress` SQL に `AND archived_at IS NULL` を追加。`useTodayTasks.js` および `dashboard/page.js` と一致する除外ロジックとなり、サイドバー進捗の正確性が回復。
2. **NG-2 修正**: `useTodayTasks.js` の `loadTasks` SQL に `AND t.status_code != 5` を追加。`layout.js` および `dashboard/page.js` と一致するキャンセル除外ロジックとなり、画面間の一貫性が確保。`TaskList.js` はキャンセル表示を意図的に維持しており、影響なし。
3. **NG-B-1 修正**: `RoutineFormModal.js` に `TaskEditModal.js` と同一パターンの Escape キーリスナーを追加。フォーム操作への干渉なし、他の Escape リスナーとの競合なし。
