...
const ACTIVE_DRAFT_KEYS = new Set(['yarukoto:work-selection']);
...
export function retireDraftsForDatabaseRestore(storage = window.localStorage, restoredAt = new Date().toISOString()) {
    ...
    const cachedEntries = snapshotWorkDraftCache();
    ...
    if (!entries.length) { clearWorkDraftCache(); return { backupKey: null, count: 0 }; }
