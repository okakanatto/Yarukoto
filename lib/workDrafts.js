    ...
    try {
        ...
        if (value && typeof value === 'object' && !Array.isArray(value)) {
            const fields = value.fields && typeof value.fields === 'object' ? Object.fromEntries(
                ['title', 'notes', 'source_ref', 'next_step', 'waiting_on', 'review_date', 'next_task_id']
                    .filter(name => typeof value.fields[name] === 'string').map(name => [name, value.fields[name]])
            ) : undefined;
            const checkpoint = value.checkpoint && typeof value.checkpoint === 'object' && !Array.isArray(value.checkpoint)
                ? Object.fromEntries(['result', 'next_step', 'stepCompleted']
                    .filter(name => typeof value.checkpoint[name] === (name === 'stepCompleted' ? 'boolean' : 'string'))
                    .map(name => [name, value.checkpoint[name]])) : undefined;
            const draft = { fields, childText: typeof value.childText === 'string' ? value.childText : '', ...(checkpoint ? { checkpoint } : {}) };
            drafts.set(id, draft);
            return draft;
        }
