...
export const TASK_CSV_FIELDS = [
    ...
    'waiting_on', 'review_date', 'next_task_id', 'last_opened_at',
];
...
export function readTaskCSV(text) {
    ...
    const rows = records.slice(1).map(cols => Object.fromEntries(headers.map((header, i) => [header, cols[i] ?? ''])))
...
export async function importTasksCSV(db, text) {
    const rows = readTaskCSV(text); // Validate every relationship before writing anything.
    ...
    const prepared = rows.map(row => {
        ...
        for (const field of ['start_date', 'due_date', 'today_date', 'notes', 'created_at', 'updated_at', 'completed_at', 'archived_at', 'capture_text', 'source_ref', 'waiting_on', 'review_date', 'last_opened_at']) {
            if (Object.hasOwn(row, field)) result[field] = row[field] || (['capture_text', 'source_ref', 'waiting_on'].includes(field) ? '' : null);
        }
