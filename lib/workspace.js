...
import { memoChangeAssignments, readWorkEntries } from '@/lib/workEntries';
...
export async function loadWorkspace() {
  ...
  const totals = await db.select(`SELECT project_id,
    ...
    SUM(CASE WHEN archived_at IS NULL AND status_code NOT IN (3, 5) THEN 1 ELSE 0 END) AS open,
    SUM(CASE WHEN archived_at IS NULL AND status_code = 2 THEN 1 ELSE 0 END) AS inProgress,
    SUM(CASE WHEN archived_at IS NULL AND status_code NOT IN (3, 5) AND (status_code = 4 OR trim(waiting_on) != '') THEN 1 ELSE 0 END) AS waiting
    FROM tasks GROUP BY project_id`);
  const historyTasks = await db.select(`SELECT t.id, t.parent_id, t.project_id, t.title, t.notes, t.work_log, t.status_code,
    t.completed_at, t.archived_at, t.due_date, t.next_task_id, t.next_step, t.waiting_on, t.review_date,
    next.title AS next_task_title, sm.label AS status_label
  ...
  for (const task of historyTasks) {
    if (task.archived_at || [3, 5].includes(Number(task.status_code))) continue;
  ...
  const branchSummary = (branch, includeDescendants = true) => {
    const members = [];
    const queue = [branch];
    const seen = new Set();
    while (queue.length) {
      ...
      if (includeDescendants) queue.push(...(childrenByParent.get(current.id) || []));
    }
    const results = members.flatMap(task => readWorkEntries(task)
      .filter(entry => entry.kind !== 'memo' && entry.result?.trim())
      .map(entry => ({ ...entry, task_id: task.id, task_title: task.title, archived_at: task.archived_at })));
    const latestResult = results.sort((a, b) => b.created_at.localeCompare(a.created_at))[0] || null;
    const unresolved = members.filter(task => !task.archived_at && ![3, 5].includes(Number(task.status_code)));
    return {
      task_id: branch.id,
      task_title: branch.title,
      latestResult,
      waiting: unresolved.filter(task => Number(task.status_code) === 4 || task.waiting_on?.trim()).map(task => ({
        task_id: task.id, task_title: task.title, status_code: task.status_code,
        waiting_on: task.waiting_on || '', review_date: task.review_date,
      })),
      nextSteps: unresolved.map(task => ({ task_id: task.id, task_title: task.title, next_step: task.next_task_title || task.next_step || '' })).filter(item => item.next_step),
    };
  };
  const milestones = historyTasks.filter(task => task.parent_id == null)
    .map(task => {
      const branches = childrenByParent.get(task.id) || [];
      return { ...task, openDescendants: openDescendants.get(task.id) || 0,
        branchSummaries: [branchSummary(task, false), ...branches.map(branch => branchSummary(branch))] };
    });
  const entriesByProject = new Map();
  for (const task of historyTasks) {
    const workEntries = readWorkEntries(task);
    const entries = workEntries.filter(entry => entry.kind !== 'memo' && (entry.result?.trim() || entry.consumed_step?.trim()));
    // Only a snapshot that still matches the current memo supplies its date.
    // Legacy/imported notes stay undated rather than borrowing updated_at.
    if (task.notes?.trim()) {
      const currentMemo = workEntries.filter(entry => entry.kind === 'memo' && entry.memo === task.notes && entry.created_at)
        .sort((a, b) => b.created_at.localeCompare(a.created_at))[0] || null;
      entries.push(currentMemo
        ? { ...currentMemo, result: task.notes }
        : { id: `memo-${task.id}`, kind: 'memo', result: task.notes, created_at: '', consumed_step: '' });
    }
  ...
  const projects = rows.map(project => ({ ...project,
    ...
    recentEntries: (() => {
      ...
      const content = entry => String(entry.result || entry.consumed_step || '').trim();
      const results = entries.filter(entry => entry.kind !== 'memo')
        .sort((a, b) => b.created_at.localeCompare(a.created_at));
      const memos = new Map(entries.filter(entry => entry.kind === 'memo').map(entry => [entry.task_id, entry]));
      const visible = [];
      const seen = new Set();
      const add = entry => {
        const key = `${entry.task_id}\u0000${content(entry)}`;
        if (!content(entry) || seen.has(key)) return;
        seen.add(key);
        visible.push(entry);
      };
      for (const entry of results) {
        // Pair the current memo with this task's newest result. Their real dates
        // decide which is the current update; an undated legacy memo is background.
        const memo = memos.get(entry.task_id);
        if (memo) {
          if (memo.created_at && memo.created_at > entry.created_at) { add(memo); add(entry); }
          else { add(entry); add(memo); }
          memos.delete(entry.task_id);
        } else add(entry);
      }
      for (const memo of [...memos.values()].sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''))) add(memo);
...
export async function saveWorkContext(id, patch) {
  ...
  const hasNotes = Object.hasOwn(patch, 'notes') && patch.notes !== undefined;
  if (hasNotes) {
    const notes = textValue(patch.notes);
    const bind = value => { values.push(value); return `$${values.length}`; };
    assignments.push(...memoChangeAssignments(bind, notes));
  }
  for (const name of ['source_ref', 'waiting_on', 'next_step']) {
  ...
  if (hasNotes) {
    values.push(context.task.notes || '');
    where += ` AND COALESCE(notes, '') = $${values.length}`;
  }
  const db = await fetchDb();
  const changed = await db.execute(`UPDATE tasks SET ${assignments.join(', ')}, updated_at = datetime('now', 'localtime') WHERE ${where}`, values);
  if (!changed.rowsAffected) throw new Error('仕事の状態が変わりました。内容を確認してもう一度保存してください');
  notifyTasksChanged();
