...
export async function loadWorkspace() {
  ...
  const branchSummary = branch => {
    ...
    const results = members.flatMap(task => readWorkEntries(task)
      .filter(entry => entry.result?.trim())
    ...
    const unresolved = members.filter(task => ![3, 5].includes(Number(task.status_code)));
    return {
      task_id: branch.id,
      task_title: branch.title,
      latestResult,
      waiting: unresolved.filter(task => task.waiting_on?.trim()).map(task => ({ task_id: task.id, task_title: task.title, waiting_on: task.waiting_on, review_date: task.review_date })),
  ...
  const milestones = historyTasks.filter(task => task.parent_id == null)
    .map(task => {
      ...
      return { ...task, openDescendants: openDescendants.get(task.id) || 0,
        branchSummaries: (branches.length ? branches : [task]).map(branchSummary) };
    });
  const entriesByProject = new Map();
  for (const task of historyTasks) {
    const entries = readWorkEntries(task).filter(entry => entry.result?.trim() || entry.consumed_step?.trim());
    // A memo is current context, separate from dated results. There is no dedicated
    // memo timestamp in the schema, so leave it undated rather than using updated_at.
    if (task.notes?.trim()) entries.push({ id: `memo-${task.id}`, kind: 'memo', result: task.notes,
      created_at: '', consumed_step: '' });
  ...
  const projects = rows.map(project => ({ ...project,
    ...
    recentEntries: (() => {
      ...
      const visible = [];
      for (const entry of entries.filter(entry => entry.kind !== 'memo').sort((a, b) => b.created_at.localeCompare(a.created_at))) {
        // Show a result with its current context. A large set of undated memos
        // must not displace every dated result from the overview.
        if (memos.has(entry.task_id)) { visible.push(memos.get(entry.task_id)); memos.delete(entry.task_id); }
        visible.push(entry);
      }
      return [...visible, ...memos.values()].slice(0, 6);
    })(),
  }));
