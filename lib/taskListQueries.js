 ...
 * @param {string} [options.searchTerm] - Search title, notes, tags, original capture, source, and next step (LIKE)
...
export function buildTaskListQuery(options) {
    ...
    // Search remembered work context, even when it was never part of the title.
    if (searchTerm) {
        const p1 = paramIndex++;
        const p2 = paramIndex++;
        const p3 = paramIndex++;
        ...
        const p7 = paramIndex++;
        const p8 = paramIndex++;
        conditions.push(`(t.title LIKE $${p1} OR t.notes LIKE $${p2} OR t.id IN (SELECT tt2.task_id FROM task_tags tt2 JOIN tags tg2 ON tt2.tag_id = tg2.id WHERE tg2.name LIKE $${p3}) OR t.capture_text LIKE $${p4} OR t.source_ref LIKE $${p5} OR t.next_step LIKE $${p6} OR t.work_log LIKE $${p7} OR t.waiting_on LIKE $${p8})`);
