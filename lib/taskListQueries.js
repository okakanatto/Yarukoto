 ...
 * @param {string} [options.searchTerm] - Search title, notes, tags, original capture, and source (LIKE)
...
export function buildTaskListQuery(options) {
    ...
    // Include the original capture and source so abbreviated titles remain discoverable.
    if (searchTerm) {
        const p1 = paramIndex++;
        const p2 = paramIndex++;
        const p3 = paramIndex++;
        const p4 = paramIndex++;
        const p5 = paramIndex++;
        conditions.push(`(t.title LIKE $${p1} OR t.notes LIKE $${p2} OR t.id IN (SELECT tt2.task_id FROM task_tags tt2 JOIN tags tg2 ON tt2.tag_id = tg2.id WHERE tg2.name LIKE $${p3}) OR t.capture_text LIKE $${p4} OR t.source_ref LIKE $${p5})`);
