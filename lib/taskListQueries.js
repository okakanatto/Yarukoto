 ...
 * @param {string} [options.searchTerm] - Search tasks by title, notes, and tag names (LIKE)
...
export function buildTaskListQuery(options) {
    ...
    // Search by title, notes, and tag names (IMP-20, ENH-8)
    if (searchTerm) {
        const p1 = paramIndex++;
        const p2 = paramIndex++;
        const p3 = paramIndex++;
        conditions.push(`(t.title LIKE $${p1} OR t.notes LIKE $${p2} OR t.id IN (SELECT tt2.task_id FROM task_tags tt2 JOIN tags tg2 ON tt2.tag_id = tg2.id WHERE tg2.name LIKE $${p3}))`);
