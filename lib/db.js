 ...
 *   8: capture / resumption context on tasks, outcome / deadline on projects
 *   9: next step / actual work start on tasks, explicit project completion
 */
...
// New migrations must not mistake an unrelated SQL failure for an existing column.
async function addColumnIfMissing(db, table, name, definition) {
...
const MIGRATIONS = [
  ...
  },
  {
    version: 8,
    migrate: async (db) => {
      // Preserve every existing title and note. Old records have no captured original.
      const taskColumns = [
      ...
      for (const [name, definition] of taskColumns) {
        await addColumnIfMissing(db, 'tasks', name, definition);
      }
      await addColumnIfMissing(db, 'projects', 'outcome', "TEXT NOT NULL DEFAULT ''");
      await addColumnIfMissing(db, 'projects', 'due_date', 'TEXT');
    }
  },
  {
    version: 9,
    migrate: async (db) => {
      await addColumnIfMissing(db, 'tasks', 'next_step', "TEXT NOT NULL DEFAULT ''");
      await addColumnIfMissing(db, 'tasks', 'work_started_at', 'TEXT');
  ...
  },
  {
    version: 10,
    migrate: async (db) => {
      // Append-only work results travel with tasks in DB backups and CSV files.
      await addColumnIfMissing(db, 'tasks', 'work_log', "TEXT NOT NULL DEFAULT '[]'");
    }
  }
];
...
async function initDb(db) {
  ...
  try {
    ...
    // Let getDb report the failure and clear its singleton so a partial migration
    // can be retried; never expose an incomplete schema as a successful startup.
    throw e;
  }
}
...
async function runAutoArchive(db) {
  try {
    ...
    // One atomic statement: a completed parent may only hide its subtree when
    // every descendant is complete/cancelled. UNION also terminates for old cycles.
    await db.execute(
      `WITH RECURSIVE descendants(root_id, id) AS (
         SELECT id, id FROM tasks
         UNION
         SELECT d.root_id, t.id FROM descendants d JOIN tasks t ON t.parent_id = d.id
       ), eligible(id) AS (
         SELECT t.id FROM tasks t
         WHERE t.archived_at IS NULL
           AND t.status_code = 3
           AND t.completed_at IS NOT NULL
           AND date(t.completed_at) <= date('now', 'localtime', '-' || $1 || ' days')
           AND NOT EXISTS (
             SELECT 1 FROM descendants d JOIN tasks child ON child.id = d.id
             WHERE d.root_id = t.id AND child.status_code NOT IN (3, 5)
           )
       )
       UPDATE tasks
       SET archived_at = datetime('now', 'localtime')
       WHERE archived_at IS NULL
         AND status_code IN (3, 5)
         AND id IN (SELECT d.id FROM descendants d JOIN eligible e ON e.id = d.root_id)`,
      [days]
    );
  } catch (e) {
