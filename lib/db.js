...
async function initDb(db) {
  ...
  try {
    ...
    if (settingsCount[0].count === 0) {
      ...
      await db.execute("INSERT OR IGNORE INTO app_settings (key, value) VALUES ($1, $2)", ['show_overdue_in_today', '1']);
    } else {
      // In case the DB exists but doesn't have the new setting yet
      await db.execute("INSERT OR IGNORE INTO app_settings (key, value) VALUES ($1, $2)", ['show_overdue_in_today', '1']);
    }
    ...
    try {
      await db.execute("ALTER TABLE tags ADD COLUMN archived INTEGER DEFAULT 0");
    } catch (e) { /* Ignore if exists */ }
    try {
    ...
    // Seed auto_archive_days setting (0 = disabled)
    await db.execute("INSERT OR IGNORE INTO app_settings (key, value) VALUES ($1, $2)", ['auto_archive_days', '0']);

    // Migration: add sort_order to tasks (IMP-4)
    let sortOrderAdded = false;
    try {
      await db.execute("ALTER TABLE tasks ADD COLUMN sort_order INTEGER DEFAULT 0");
      sortOrderAdded = true;
    } catch (e) { /* Column already exists */ }

    // Migration: add today_sort_order to tasks (IMP-4)
    try {
      await db.execute("ALTER TABLE tasks ADD COLUMN today_sort_order INTEGER DEFAULT 0");
    } catch (e) { /* Column already exists */ }

    // Migration: add today_sort_order to routines (IMP-4)
    try {
      await db.execute("ALTER TABLE routines ADD COLUMN today_sort_order INTEGER DEFAULT 0");
    } catch (e) { /* Column already exists */ }

    // Initialize sort_order for existing tasks when column is first added
    if (sortOrderAdded) {
      // Root tasks: order by created_at DESC (newest first = lowest sort_order)
      ...
      // Child tasks: order within each parent by due_date ASC NULLS LAST, created_at ASC
    ...
    // Seed auto_complete_parent setting (ENH-5)
    await db.execute("INSERT OR IGNORE INTO app_settings (key, value) VALUES ($1, $2)", ['auto_complete_parent', '0']);

    // Migration: projects table and project_id columns (ENH-2)
    // Seed default "Inbox" project (IMP-25: renamed from "General")
    ...
    // Migration: rename default project "General" → "Inbox" (IMP-25)
    ...
    // Migration: add project_id to routines
    try {
      await db.execute("ALTER TABLE routines ADD COLUMN project_id INTEGER");
      // Assign existing routines to default project
    ...
    // Seed theme settings (IMP-8 v2.0.0)
