        ...
        try {
            ...
            if (filterStatus) {
                ...
                if (codes.length > 0) {
                    ...
                    tConditions.push(`t.status_code IN (${placeholders})`);

                    // For routines, completion_date presence determines status (3 if present, 1 if not)
                    if (codes.includes(3) && !codes.includes(1)) {
                        rConditions.push(`rc.completion_date IS NOT NULL`);
                    } else if (codes.includes(1) && !codes.includes(3)) {
                        rConditions.push(`rc.completion_date IS NULL`);
                    } else if (!codes.includes(1) && !codes.includes(3)) {
                        // If filtered by status 2, 4, 5 etc, routines NEVER match
                        rConditions.push(`1 = 0`);
                    }
                    sqlParams.push(...codes);
                }
            }
            ...
            if (filterTag) {
                tConditions.push(`t.id IN (SELECT task_id FROM task_tags WHERE tag_id = $${paramIndex++})`);
                sqlParams.push(filterTag);

                rConditions.push(`r.id IN (SELECT routine_id FROM routine_tags WHERE tag_id = $${rParamIndex++})`);
                rSqlParams.push(filterTag);
            }
