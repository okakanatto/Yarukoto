...
import { AlertCircle, ArrowLeft, CalendarDays, Check, ChevronRight, FolderOpen, RotateCcw, Settings } from 'lucide-react';
import TaskList from '@/components/TaskList';
import { TaskStatus, TaskDue, EffortValue } from '@/components/TaskVisual';
...
function ProjectPageInner() {
    ...
    return <div className="work-page">
        <header className="work-heading"><div>{project && <Link className="work-text-link" href="/projects"><ArrowLeft size={14} />プロジェクト一覧</Link>}<h1>{project?.name || 'プロジェクト'}</h1></div><Link className="work-text-link" href="/settings?tab=projects"><Settings size={15} />設定</Link></header>
...
function ProjectWorkspace({ project, tasks, reload }) {
    ...
    return <>
        ...
        {view === 'progress' ? <section id="project-panel" role="tabpanel" aria-labelledby="project-tab-progress">
            <div className={project.recentEntries?.length ? styles.progressColumns : styles.progressSingle}>
            <div>
            {milestones.length ? <>
                <div className={styles.listHeading}><h2>主な仕事</h2><span>状態</span><span>期限</span><span>見積</span></div>
            ...
            <div className={styles.progressFooter}><ProjectProgress progress={project.progress} /><button className="work-button" onClick={() => setView('tasks')}>タスクを管理 <ChevronRight size={14} /></button></div>
        ...
        <ul className={styles.entries}>{visible.map((entry, index) => <li key={`${entry.task_id}-${entry.id || entry.created_at}-${index}`}>
            <button className={styles.entry} onClick={() => openTask(entry.task_id)}>
                <span className={styles.entryBody}>
                    ...
                    <span className={styles.entrySource}>{entry.task_title}<span className={styles.meta}>{['note', 'memo'].includes(entry.kind) ? '現在のメモ' : entry.kind === 'step' ? '作業記録' : entry.kind === 'pause' ? '中断' : '記録'}{entry.created_at && ` · ${entry.created_at.slice(0, 10)}`}{entry.archived_at && ' · アーカイブ'}</span></span>
                </span>
                <ChevronRight size={15} className={styles.openIcon} aria-hidden="true" />
            </button>
        </li>)}</ul>
    </section>;
}
...
function MilestoneList({ tasks }) {
    ...
    return <ul className={styles.milestones}>{tasks.map(task => {
        ...
        return <li key={task.id}><button className={styles.milestone} onClick={() => openTask(task.id)}>
            ...
            <span className={styles.milestoneBody}><span className={styles.taskHeading}><strong>{task.title}</strong><span className={styles.taskStatus}><TaskStatus code={task.status_code} label={statusName(task)} color={task.status_color} /></span><span className={styles.taskDue}><TaskDue date={task.due_date} done={!isOpen(task) && !inconsistent} /></span><span className={styles.taskEstimate}><EffortValue minutes={task.estimated_hours} /></span></span>{legacyNote && <span className={styles.note}>{legacyNote}</span>}<BranchSummaries branches={task.branchSummaries || []} taskId={task.id} />{(task.completed_at || task.archived_at) && <span className={styles.meta}>{task.completed_at && '完了 ' + task.completed_at.slice(0, 10)}{task.archived_at && ' · アーカイブ'}</span>}{inconsistent && <span className={styles.unfinished}><AlertCircle size={13} />配下に未完了 {task.openDescendants}件</span>}</span>
            <ChevronRight size={15} className={styles.openIcon} aria-hidden="true" />
        </button></li>;
    })}</ul>;
}

function BranchSummaries({ branches, taskId }) {
    ...
    return <span className={styles.branchSummaries}>{visible.map(branch => <span key={branch.task_id} className={styles.branchSummary}>
        {branches.length > 1 && branch.task_id !== taskId && <span className={styles.branchTitle}>{branch.task_title}</span>}
        {branch.latestResult && <span className={styles.branchLine}>結果: {branch.latestResult.result}<small>{branch.latestResult.task_id !== taskId && branch.latestResult.task_id !== branch.task_id && branch.latestResult.task_title}{branch.latestResult.created_at && ` · ${branch.latestResult.created_at.slice(0, 10)}`}{branch.latestResult.archived_at && ' · アーカイブ'}</small></span>}
        {branch.waiting?.map(item => <span key={`waiting-${item.task_id}`} className={styles.branchLine}>待ち: {item.waiting_on}{item.review_date && ` · 確認 ${item.review_date}`}{item.task_id !== taskId && item.task_id !== branch.task_id && <small>{item.task_title}</small>}</span>)}
        {branch.nextSteps?.map(item => <span key={`next-${item.task_id}`} className={styles.branchLine}>次: {item.next_step}{item.task_id !== taskId && item.task_id !== branch.task_id && <small>{item.task_title}</small>}</span>)}
    </span>)}</span>;
}
...
function ProjectContext({ project, onChanged }) {
    ...
    return <section className={`project-context ${!editing && !dirty && !error && !project.outcome && !project.due_date && !project.completed_at ? 'project-context-empty' : ''}`} aria-label="プロジェクトの目標と期限">
        <div className="project-context-summary">{(project.outcome || project.due_date || project.completed_at) && <div>{project.outcome && <><small>目標</small><p>{project.outcome}</p></>}<div className="work-row-meta">{project.due_date && <span><CalendarDays size={14} />期限 {project.due_date}</span>}{project.completed_at && <span className={styles.completeStatus}><Check size={14} />完了 {project.completed_at.slice(0, 10)}</span>}{project.completed_at && project.progress?.open > 0 && <span>完了後の未完了 {project.progress.open}件</span>}</div></div>}
            <div className={styles.contextActions}><button className="work-button" disabled={saving} onClick={() => setEditing(!editing)}>{editing ? '閉じる' : dirty ? '下書きを再開' : project.outcome || project.due_date ? '編集' : '目標・期限を設定'}</button>
        ...
        {editing && <form className="project-context-form" onSubmit={save}>
            <label>目標<textarea rows={2} placeholder="例：移行方式を決め、関係者の合意を得る" value={outcome} onChange={event => draft.update('outcome', event.target.value)} disabled={saving} /></label>
