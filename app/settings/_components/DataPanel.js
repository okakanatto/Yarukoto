...
import { exportTasksCSV, importTasksCSV } from '@/lib/csv';
import { notifyTasksChanged } from '@/lib/taskHierarchy';
import { retireDraftsForDatabaseRestore } from '@/lib/drafts';
...
export default function DataPanel({ flash }) {
    ...
    const handleRestore = async () => {
        try {
            ...
            try {
                retireDraftsForDatabaseRestore();
            } catch {
                // Do not expose old pending IDs to the replacement database.
                try {
                    await copyFile(backupPath, dbPath);
                    flash('err', '下書きの退避に失敗したため、データベースを元に戻しました。');
                } catch {
                    flash('err', '下書きの退避とデータベースの差し戻しに失敗しました。操作を続けず、自動バックアップから復元してください。');
                }
                return;
            }
            ...
            try {
                ...
                flash('err', 'データベースは復元しました。アプリを終了して起動し直してください。');
            }
    ...
    const handleExport = async () => {
        try {
            ...
            const rows = await db.select(`
                ...
                ORDER BY t.id
            `);
            const csvText = exportTasksCSV(rows);
    ...
    const handleImport = async (ev) => {
        ...
        try {
            ...
            const db = await fetchDb();
            const count = await importTasksCSV(db, text);
            flash('ok', `${count}件のタスクをインポートしました`);
        } catch (e) {
            console.error(e);
            flash('err', e.message || 'インポートに失敗しました');
        } finally {
            ev.target.value = '';
        }
    };
