    ...
    return <div className={`${styles.workspace} ${fullPage ? styles.fullPage : ''} ${editingId ? styles.withDetail : ''} ${focused ? styles.focused : ''}`}>
        <section className={styles.list} aria-label="タスク管理">
            ...
            <div className={styles.toolbar}>
                ...
                <select aria-label="一覧の構成" value={view.group} onChange={event => patchView({ group: event.target.value })}><option value="tree">階層</option><option value="project">プロジェクト別</option><option value="status">状態別</option></select>
        ...
        {editingId && <aside className={styles.detail} aria-label="選んだタスク"><div className={styles.detailTop}><button onClick={startWork}><ArrowRight size={15} />仕事で取りかかる</button></div><WorkDetailPanel embedded navigationRef={navigation} taskId={editingId} onClose={() => open(null)} onChanged={reload} onOpenTask={open} onFocusChange={setFocused} /></aside>}
