...
import { TaskStatus, TaskDue } from './TaskVisual';
            ...
            <div className={styles.body} ref={bodyRef}>
                ...
                {task && draft && <>
                    ...
                    {!checkpoint && !startHelp && <>{active && !focused && <div className={styles.workActions}>
                        <button type="button" className={styles.primaryButton} disabled={saving || !!checkpoint} onClick={() => startWork(null, reference.kind !== 'text')}><Play size={14} />{reference.kind !== 'text' ? 'リンクを開いて開始' : Number(task.status_code) === 2 ? '再開' : '作業を開始'}</button>
                        <button type="button" className={styles.secondaryButton} disabled={saving || !!checkpoint} onClick={() => startWork(5)}><Clock3 size={14} />5分だけ</button><button type="button" className={styles.textButton} disabled={saving} onClick={() => setStartHelp(true)}>着手のヒント</button>
                    </div>}
                    {focused && <div className={styles.focusBar}><span><span className={styles.liveDot} />作業中{sessionMinutes && <time aria-label="作業の経過時間">{Math.floor(elapsed / 60)}:{String(elapsed % 60).padStart(2, '0')} / {sessionMinutes}:00</time>}</span><button type="button" className={styles.secondaryButton} disabled={saving || !!checkpoint} onClick={() => openCheckpoint()}><Pause size={14} />中断</button></div>}

                    {focused && entryStep && <div className={styles.entryStep}><small>最初に試すこと</small><p>{entryStep}</p><button type="button" onClick={() => setEntryStep('')}>次の行動へ<ArrowRight size={14} /></button></div>}
                    <div className={styles.referenceLine}>
                        <Paperclip size={15} aria-hidden="true" />
                        {sourceEditing ? <><input aria-label="関連リンク" autoFocus value={draft.source_ref} disabled={saving} placeholder="URLまたはファイルパス" onChange={event => changeField('source_ref', event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); setSourceEditing(false); } }} /><button className={styles.iconButton} aria-label="リンクの編集を終える" type="button" disabled={saving} onClick={() => setSourceEditing(false)}><Check size={16} /></button></> : draft.source_ref ? <>{reference.kind !== 'text' ? <button type="button" className={styles.referenceLink} disabled={saving} onClick={openReference} title={draft.source_ref}>{reference.label}<ArrowUpRight size={14} /></button> : <span className={styles.referenceText}>{draft.source_ref}</span>}<button type="button" className={styles.iconButton} aria-label="リンクを編集" title="リンクを編集" disabled={saving} onClick={() => setSourceEditing(true)}><Pencil size={14} /></button></> : <button type="button" className={styles.textButton} disabled={saving} onClick={() => setSourceEditing(true)}>関連リンクを追加</button>}
                    </div>

                    <section className={styles.nextStep}>
                        {nextTask ? <><span className={styles.stepLabel}>次の行動</span><button type="button" className={styles.nextTaskLink} disabled={saving} onClick={() => { if (draftRef.current.next_step) chooseChild(String(nextTask.id)); void leave(() => callbacks.current.onOpenTask(nextTask.id)); }}>{nextTask.title}<ArrowRight size={16} /></button><button type="button" className={styles.textButton} disabled={saving} onClick={() => chooseChild('')}>次の行動を変更</button></> : <><label htmlFor={`work-step-${taskId}`}>{entryStep ? '予定していた行動' : '次の行動'}</label><textarea id={`work-step-${taskId}`} rows={1} value={draft.next_step} placeholder="次にすること（任意）" disabled={saving} onChange={event => changeField('next_step', event.target.value)} /></>}
                        {focused && !entryStep && !nextTask && draft.next_step.trim() && <button type="button" className={styles.stepDone} disabled={saving || !!checkpoint} onClick={() => openCheckpoint(true)}><Check size={15} />完了を記録</button>}
                    </section></>}

                    {startHelp && !checkpoint && <section className={styles.startHelp} aria-label="着手のヒント">
                    ...
                    <section className={styles.notesSection}>
                        ...
                        {selectedText && <div className={styles.selectionActions}><span>選択した文</span><button type="button" className={styles.textButton} disabled={saving} onClick={useSelectionAsStep}><ArrowRight size={13} />次の行動にする</button><button type="button" className={styles.textButton} disabled={saving} onClick={selectedTextToChild}><Plus size={13} />子タスクにする</button></div>}
                    </section>
                    {task.notes && renderOriginal()}
                    {nextTask && draft.next_step && <details className={styles.originalSection}><summary>以前の行動</summary><p className={styles.original}>{draft.next_step}</p></details>}
                    ...
                    <details className={styles.section} open={childOpen || undefined}>
                        ...
                        {(descendants.length > 0 || draft.next_task_id) && <div className={styles.nextRow}><label htmlFor={`work-next-${taskId}`}>次に進める子タスク</label><select id={`work-next-${taskId}`} value={draft.next_task_id} disabled={saving} onChange={event => chooseChild(event.target.value)}>
